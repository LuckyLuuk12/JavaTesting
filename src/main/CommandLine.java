...
public class CommandLine {
  public String input = "";
  public QueueStack qs = new QueueStack();
  public GraphList dg = new GraphList(8);
  public Crypto c = new Crypto();
  ...
  public void prompt() {
    while(!input.equalsIgnoreCase("exit")) {
      ...
      String[] commands = input.split(" \\| ");                                 // command list using '|' as seperator
      executeCommands(commands);                                                    // execute all commands
  ...
  public void executeCommands(String[] commands) {
    ...
    else if(pars[0].toLowerCase().contains("fac") && pars.length > 1) { System.out.println("> fac "+pars[1]+" = "+fac(Long.parseLong(pars[1]))); }
    else if(pars[0].toLowerCase().contains("qs-push ") && pars.length > 1) {
      qs.push(pars[1]);
      System.out.println("> Pushed "+pars[1]+" to the QueueStack");
    }
    else if(pars[0].toLowerCase().contains("qs-pop")) { System.out.println("> Popped "+qs.pop()+" from the QueueStack"); }
    else if(pars[0].toLowerCase().contains("qs-show") || pars[0].toLowerCase().contains("print stack")) { qs.show(); }
    else if(pars[0].toLowerCase().contains("dg-sizeandweight") && pars.length > 2) {
      dg = new GraphList(Integer.parseInt(pars[1]), Boolean.parseBoolean(pars[2]));
    }
    else if(pars[0].toLowerCase().contains("dg-set") && pars.length > 2) {
      boolean done = dg.setEdge(Integer.parseInt(pars[1]), Integer.parseInt(pars[2]));
      if(done) System.out.println("> Set an edge from "+pars[1]+" to "+pars[2]);
    }
    else if(pars[0].toLowerCase().contains("dg-wset") && pars.length > 3) {
      boolean done = dg.setWeightedEdge(Integer.parseInt(pars[1]), Integer.parseInt(pars[2]), Integer.parseInt(pars[3]));
      if(done) System.out.println("> Set an edge from "+pars[1]+" to "+pars[2]+" weighted "+pars[3]);
    }
    else if(pars[0].toLowerCase().contains("dg-setlist") && pars.length > 1) {
      for(String[] s : to2dArray(pars[1])) dg.setEdge(Integer.parseInt(s[0]), Integer.parseInt(s[1]));
    }
    else if(pars[0].toLowerCase().contains("dg-show")) { dg.show(); dg.showPredecessors(); }
    else if(pars[0].toLowerCase().contains("dg-size") && pars.length > 1) { dg = new GraphList(Integer.parseInt(pars[1])); }
    else if(pars[0].toLowerCase().contains("dg-sink")) { System.out.println(dg.detectSink()); }
    else if(pars[0].toLowerCase().contains("dg-idfs")) {
      if(pars.length > 1) dg.iterativeDFS(Integer.parseInt(pars[1])); else dg.iterativeDFS(0);
    }
    else if(pars[0].toLowerCase().contains("dg-rdfs")) {
      if(pars.length > 1) dg.recursiveDFS(Integer.parseInt(pars[1])); else dg.recursiveDFS(0);
    }
    else if(pars[0].toLowerCase().contains("dg-mother")) { System.out.println("> Mothervertex: "+dg.hasMotherVertex()); }
    else if(pars[0].toLowerCase().contains("dg-dijkstra")) {
      if(pars.length > 1) dg.dijkstra(Integer.parseInt(pars[1])); else dg.dijkstra(0);
    }
    else if(pars[0].toLowerCase().contains("dg-bf")) {
      if(pars.length > 1) dg.bellmanFord(Integer.parseInt(pars[1])); else dg.bellmanFord(0);
  ...
  public String[][] to2dArray(String s) {
